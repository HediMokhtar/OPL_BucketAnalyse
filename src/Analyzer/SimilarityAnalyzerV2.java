            ...
            Thread thread = new Thread(new Runnable() {
                public void run() {
                    ...
                    for (Stacktrace stacktrace : bucket) {
                        ...
                        double functionSimilarity = 0;
                        for(int i = 0; i < stacktrace.size(); i++)
                            functionSimilarity = similarity(stackTrace.get(i).getFunctionName(), stacktrace.get(i).getFunctionName());

                        double fileSimilarity = similarity(stackTrace.getFullString(), stacktrace.getFullString());
                        double librarySimilarity = similarity(stackTrace.getFullString(), stacktrace.getFullString());
                        globalSimilarity +=
                    ...
                    if (result < globalSimilarity / stacktraceNumber) {
                        result = globalSimilarity / stacktraceNumber;
